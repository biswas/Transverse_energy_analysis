...
int fitBESData4_oneByOne_6Params(){
	...
	while((mikey=(TKey*)next())){// FIXME!!! delete mikey at the end of every loop
		...
		funcBGBW = new TF1("getdNdpt",getdNdpt,0.00000000000001,10.,6);
		funcBGBW_top = new TF1("getdNdpt_top",getdNdpt,0.00000000000001,10.,6);
		funcBGBW_bottom = new TF1("getdNdpt_bottom",getdNdpt,0.00000000000001,10.,6);
		...
		dNdEtaIntegrandFunc = new TF1("dETdyIntegrand",
								  getdNdEtaIntegrand,
								  0,10,6); // 5 parameters:m,b,t,n,norm
		...
		funcBGBW_top->SetParameters(mass,beta_top,temp_top,n_top,norm_top,type);
		funcBGBW_top->Draw("SAME");
		funcBGBW_top->SetLineColor(4);
		funcBGBW_top->SetLineStyle(2);
		funcBGBW_bottom->SetParameters(mass,beta_bottom,temp_bottom,n_bottom,norm_bottom,type);
