...
/// added functionality to redo fit with different set of initial parameters if first fit is not successful; now this code is more generic, i.e., no need to specify histogram name before compile time in order to try the second set of parameters
...
int fitAllHistosInTFile(){
	...
	int stop =71; // breakOut after this many iterations (if achieved); default: 140
	while((mikey=(TKey*)next())){// TODO!!! delete mikey at the end of every loop
		...
		int avoidInfLoop = 1;
		while(avoidInfLoop<7){ // 5 for trying 4 different sets of initial parameters
								// all of which may not have been created below
			r = h->Fit("getdNdpt","S","",0.00000000000001,7.);
			if(r->IsValid()) break;
			else{
				cout << "Trying alternative set of parameters..." << endl;
				r = 0;
				TFitResultPtr r;
				funcBGBW->
					SetParameters(mass,0.9,(0.01*avoidInfLoop),
					(0.01*avoidInfLoop),(10000.*avoidInfLoop),type);
				cout << "So far no error of pointing to invalid address..." << endl ;
			}
			avoidInfLoop++;
		}
