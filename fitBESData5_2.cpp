// what's different from fitBESData4_1.cpp:
	...
	// put the following immediately after while loop condition line: checked
		// breakOutForTesting++;
		// if (breakOutForTesting<141) continue;// 140 histograms already analyzed
	// get rid of breakOutForTesting++ at the end of while loop: checked
	// initialize int stop to 280 instead of 140 just before the while loop: checked!
	...
	*/
	...
	while((mikey=(TKey*)next())){// TODO!!! delete mikey at the end of every loop
		...
		if (	histoName == "cent7_ka-_Au+Au_7.7"
			...
			funcBGBW->SetParameters(0.1,0.9,0.03,0.01,10000.,type);
			}			
		else{
			funcBGBW->SetParameters(0.1,0.95,0.05,0.1,1000000.,type);
			}
