...
void outputDatFile(Double_t* fittedParams, string partclID);//FIXME
...
int fitBESData(){
	...
	datFile << "E" << "\t"	//TODO:collision energy
			<< "part" << "\t"
			<< "mass" << "\t"
			<< "beta" <<"\t"
			<< "beta_err" <<"\t"
	...
	while((mikey=(TKey*)next())){// FIXME!!! delete mikey at the end of every loop
		TCanvas* c1 = new TCanvas; // FIXME declare outside loop scope if clear method to be invoked
		gPad->SetLogy();
		...
		if(!class1->InheritsFrom("TH1")){ /*FIXME delete all objects in memory before continuing*/continue;}
		...
		//FIXME when you delete, use the "C"? option to delete all the inherited objects as well
 		...
 		// FIXME!! define function outside the loop and feed the parameters inside
		...
		// FIXME: define this function outside the loop as well:
		...
		datFile << 0.0 << "\t"	//TODO:collision energy
		<< particleID << "\t"
		<< mass << "\t"
		<< beta <<"\t"
		<< betaErr <<"\t"
		...
		delete h;
		delete c1;// FIXME!!! use clear method instead of delete at the end of every loop
		/// sometimes when you delete objects, they stay in the program heap
		delete png;
		breakOutForTesting++;
